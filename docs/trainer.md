# Trainer Notes

## Day 1 - ETL Locally

### Session 1

- `10:10` [Introducing HomeSphere](day1/homesphere.md) (30 mins)

### Session 2

- `11:00` **Practice**: [Open Sales Data](day1/inspect-sales.md) (30 mins)
- `11:30` **Practice**: [Open Product Data](day1/inspect-prod.md) (10 mins)
- `11:40` **Discussion**: [What Did You Find?](day1/inspect-debrief.md) (20 mins)
- `12:00` [Cleaning Moves Overview](day1/clean-intro.md) (20 mins)

### Session 3

- `13:20` **Practice**: [Clean the Sales Data](day1/clean.md) (50 mins)
- `14:10` **Discussion**: [Mechanical or Judgement?](day1/clean-debrief.md) (20 mins)

### Session 4

- `14:50` [Why Flatten and Join?](day1/flatten-intro.md) (10 mins)
- `15:00` **Practice**: [Flatten and Join](day1/join.md) (30 mins)
- `15:30` **Practice**: Which Categories Earn Most? (10 mins)
- `15:40` **Discussion**: [Reflect and Look Ahead](day1/reflection.md) (10 mins)

---

## Day 2 - ETL in the Cloud

### Session 1

- `09:40` What Is Fabric? (20 mins)
- `10:00` **Investigation**: Explore the Fabric Environment (30 mins)
- `10:30` **Discussion**: What Stays the Same? (10 mins)

### Session 2

- `11:00` [Landing Raw Data](day2/land-data.md) (20 mins)
- `11:20` **Practice**: [Clean and Flatten in Fabric](day2/cloud-clean.md) (50 mins)
- `12:10` **Discussion**: [Familiar vs Different?](day2/cloud-debrief.md) (10 mins)

### Session 3

- `13:20` **Practice**: [Build the Trusted Output](day2/cloud-output.md) (50 mins)
- `14:10` **Discussion**: [What Is Better? What Is Fragile?](day2/output-debrief.md) (20 mins)

### Session 4

- `14:50` Security, Scalability, Governance, Cost (10 mins)
- `15:00` **Breakout**: Local vs Cloud (20 mins)
- `15:20` **Report-Back**: Share Your Comparison (10 mins)
- `15:30` **Discussion**: Cloud Is Not Well-Architected (20 mins)

---

## Day 3 - Robust & Well-Architectured

### Session 1

- `09:40` Design Review (20 mins)
- `10:00` **Breakout**: Where Are the Weaknesses? (20 mins)
- `10:20` **Report-Back**: Our Design Debts (20 mins)

### Session 2

- `11:00` [Bronze, Silver, Gold](day3/medallion/group-scenarios.md) (30 mins)
- `11:30` **Practice**: Map the Pipeline (30 mins)
- `12:00` **Discussion**: What Needs to Move? (20 mins)

### Session 3

- `13:20` **Practice**: Refactor the Pipeline (50 mins)
- `14:10` **Discussion**: What Is Actually Better? (20 mins)

### Session 4

- `14:50` Extension Source Task (10 mins)
- `15:00` **Breakout**: Design B/S/G for Your Source (20 mins)
- `15:20` **Report-Back**: Share Your Design (20 mins)
- `15:40` **Discussion**: Bridge to Day 4 (10 mins)

---

## Day 4 - Supportable & Explainable

### Session 1

- `09:40` **Discussion**: Silent Failures (20 mins)
- `10:00` **Practice**: Which Checks Matter? (20 mins)
- `10:20` **Practice**: Add Validation Checks (20 mins)

### Session 2

- `11:00` Minimum Viable Documentation (20 mins)
- `11:20` **Practice**: Create a Handover Artefact (40 mins)
- `12:00` **Breakout**: Review Each Other's Artefact (20 mins)

### Session 3

- `13:20` Technical vs Stakeholder View (20 mins)
- `13:40` **Breakout**: Prepare Your Explanation (20 mins)
- `14:00` **Report-Back**: Share Your Explanation (20 mins)
- `14:20` **Discussion**: What Made the Difference? (10 mins)

### Session 4

- `14:50` **Discussion**: Next Maturity Steps (20 mins)
- `15:10` **Discussion**: Module Retrospective (30 mins)

---
